                ...
                <tr v-for="(row, index) in filteredAndPaginatedData" :key="index">
                  ...
                  <td>
                    ...
                    <ul class="dropdown-menu p-2 gap-3">
                      ...
                      <li v-if="dataPerfil.rol?.id !== 2"><button class="btn btn-outline-warning text-red dropdown-item fs-5 p-0" title="Exportar" type="button" @click="fileData(row,'export',(row.nombre) )"><i class="bi bi-upload"></i>Exportar</button></li>
