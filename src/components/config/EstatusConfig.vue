...
import EliminarModalEstatus from '../modales/administrativo/estatus/EliminarModalEstatus.vue';
...
const handleData = async (action = null, params = null, id = null) => {
  try {
    if (action === 'create' || action === 'update') {
      ...
      response.value = action === 'create' ? await EstatusServicios(action, id, params)
        : action === 'update' ? await EstatusServicios(action, id, params) : undefined;
      await handleData('fetchAll');
    } else if (action === 'delete') {
      await EstatusServicios(action, id);
      await handleData('fetchAll');
    } else if (action === 'fetch') {
      paramsE.value = await EstatusServicios('fetch', id);
    }
  } catch (error) {
...
const fileData = async (fileEvent,format,nameFile) => {
  switch (format) {
    ...
    case 'import':
      ...
      try {
          response.value = await EstatusServicios(format, '', formData);
          await handleData('fetchAll');
      } catch (error) {
...
<template>
  <div>
              ...
              <thead class="">
                <tr class="">
                  <div class="p-2 text-nowrap" v-if="dataPerfil.rol?.id !== 2 && dataPerfil.rol?.id !== 3 && dataPerfil.rol?.id !== 4"><i class="bi bi-ui-checks"></i></div>
                    <div class="form-check form-switch d-flex justify-content-end px-3" style="height: 60px;" v-if="dataPerfil.rol?.id !== 2 && dataPerfil.rol?.id !== 3 && dataPerfil.rol?.id !== 4">
    ...
    <AgregarModalEstatus :handleData="handleData" :isLoadingImport="isLoadingImport" :response="response" />
    ...
    <ImportarModalEstatus :isLoadingImport="isLoadingImport" :fileData="fileData" :response="response" />
