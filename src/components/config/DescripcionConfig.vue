...
import EliminarModaDescripcion from '../modales/inventario/descripcion/EliminarModaDescripcion.vue';
...
const handleData = async (action = null, params = null, id = null) => {
  try {
    ...
    : action === 'update'? await DescripcionServicios(action, id, params):undefined;
      ...
      await DescripcionServicios(action, id);
      await handleData('fetchAll');
    } else if (action === 'fetch') {
      paramsE.value = await DescripcionServicios('fetch', id);
    }
  } catch (error) {
...
const fileData = async (fileEvent,format,nameFile) => {
  switch (format) {
    ...
    case 'import':
      ...
      try {
          response.value = await DescripcionServicios(format, '', formData);
          await handleData('fetchAll');
      } catch (error) {
...
<template>
  <div>
                ...
                <div class="col-6 btn-group" v-if="dataPerfil.rol?.id !== 2 && dataPerfil.rol?.id !== 4">
                  ...
                  <div class="dropdown">
                    ...
                    <ul class="dropdown-menu">
                      ...
                      <li v-if="dataPerfil.rol?.id !== 3"><button class="dropdown-item" type="button" @click="fileData(rowData,'exportAll','descripcion')"><i class="bi bi-upload"></i> Exportar</button></li>
    ...
    <AgregarModalDescripcion :handleData="handleData" :relations="relations" :isLoadingImport="isLoadingImport" :response="response" />
    ...
    <ImportarModalDescripcion :isLoadingImport="isLoadingImport" :fileData="fileData" :response="response" />
