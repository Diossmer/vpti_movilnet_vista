...
import EliminarModalProductos from '../modales/inventario/productos/EliminarModalProductos.vue';
                ...
                <tr v-for="(row, index) in filteredAndPaginatedData" :key="index">
                  ...
                  <td>
                    ...
                    <ul class="dropdown-menu p-2 gap-3">
                      ...
                      <li v-if="dataPerfil.rol?.id !== 2 && dataPerfil.rol?.id !== 3 && dataPerfil.rol?.id !== 4"><button class="btn btn-outline-warning text-red dropdown-item fs-5 p-0" title="Exportar" type="button" @click="fileData(row,'export',(row.usuario?.usuario) )"><i class="bi bi-upload"></i>Exportar</button></li>
