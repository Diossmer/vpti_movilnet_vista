...
const handleData = async (action, data = null) => {
  try {
    ...
    if (action === 'create' || action === 'update') {
      ...
      response = action=== 'create'? await RolesServicios(action, data, paramsA.value)
    : action === 'update'? await RolesServicios(action, data, paramsE.value):undefined;
      await handleData('fetchAll');
      paramsA.value = {};
      paramsE.value = {};
    } else if (action === 'delete') {
      await RolesServicios(action, data);
      await handleData('fetchAll');
    } else if (action === 'fetch') {
      paramsE.value = await RolesServicios('fetch', data);
    }
...
watch([() => paramsE.value?.nombre,
() => paramsE.value?.descripcion], ([nombre, descripcion]) => {
  const errors = [];
  const nombreError = validacionesUtils().nombreValid(nombre);
  if (nombreError) errors.push(nombreError);
  const descripcionError = validacionesUtils().textareaValid(descripcion);
...
const fileData = async (fileEvent,format,nameFile) => {
  switch (format) {
    ...
    case 'import':
      ...
      try {
          const response = await RolesServicios(format, '', formData);
          await handleData('fetchAll');
          avisos.value=response;
      } catch (error) {
  ...
  <div>
                ...
                <div class="modal-body">
                  <div class="row">
                    <div class="col-6">
                      ...
                      <input type="text" pattern="^[A-Za-zÁ-Úá-úñÑ\s\d]+$" class="form-control" :class="{'is-invalid':paramsA.nombre && !/^[A-Za-zÁ-Úá-úñÑ\s\d]+$/.test(paramsA.nombre),'is-valid':paramsA.nombre && /^[A-Za-zÁ-Úá-úñÑ\s\d]+$/.test(paramsA.nombre)}" v-model="paramsA.nombre" placeholder="Nombre" required />
                    </div>
                    <div class="col-6">
    ...
    <div class="modal fade" id="staticMostrar" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
      <div class="modal-dialog modal-lg modal-dialog-scrollable modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <h1 class="modal-title fs-5 fw-bolder" id="staticBackdropLabel">roles</h1>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" @click="paramsE = {}, avisos = null, avisosAlert =''"></button>
          </div>
          <div class="modal-body">
            <div class="row">
