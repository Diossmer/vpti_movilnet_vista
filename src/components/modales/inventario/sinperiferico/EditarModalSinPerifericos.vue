...
() => props.paramsE?.observacion], ([/* cantidad_existente, */ entrada, salida, observacion]) => {
  const errors = [];
...
<template>
  <div class="modal fade" id="staticEditar2" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" ref="modalEditar">
            ...
            <template #default>
              <form @submit.prevent="handleData('update', paramsE, paramsE.id)">
                <div class="modal-body">
                  <div class="row">
                    <span class="text-red">cantidad seleccionada: <b>{{ paramsE.descripcion_id.length }}</b></span>
                    <div class="col-12">
                    ...
                    <div class="col-12">
                      ...
                      <textarea class="form-control" :class="{ 'is-invalid': paramsE.observacion && !/^[^<>{}\[\]]+$/.test(paramsE.observacion), 'is-valid':paramsE.observacion && /^[^<>{}\[\]]+$/.test(paramsE.observacion)}" placeholder="Descripción" v-model="paramsE.observacion"></textarea>
                    </div>
