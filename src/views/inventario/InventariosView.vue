...
import EliminarModalSinPerifericos from '@/components/modales/inventario/sinperiferico/EliminarModalSinPerifericos.vue';
...
const handleData = async (action = null, params = null, id = null) => {
  try {
    if (action === 'create' || action === 'update') {
      ...
      response.value = action=== 'create'? await InventariosServicios(action, id, params)
    : action === 'update'? await InventariosServicios(action, id, params):undefined;
      await handleData('fetchAll');
    } else if (action === 'delete') {
      await InventariosServicios(action, id);
      await handleData('fetchAll');
    } else if (action === 'fetch') {
      paramsE.value = await InventariosServicios('fetch', id);
    }
  } catch (error) {
...
const fileData = async (fileEvent,format,nameFile) => {
  switch (format) {
    ...
    case 'import':
      ...
      try {
          response.value = await InventariosServicios(format, '', formData);
          await handleData('fetchAll');
      } catch (error) {
...
<template>
  <div>
                ...
                <div class="col-6 btn-group" v-if="dataPerfil.rol?.id !== 4">
                  ...
                  <div class="dropdown">
                    ...
                    <ul class="dropdown-menu">
                      ...
                      <li v-if="dataPerfil.rol?.id !== 3"><button class="dropdown-item" type="button" @click="fileData(rowData,'exportAll','inventarios')"><i class="bi bi-upload"></i> Exportar</button></li>
                ...
                <tr v-for="(row, index) in filteredAndPaginatedData" :key="index">
                  ...
                  <td>
                    ...
                    <ul class="dropdown-menu p-2 gap-3">
                      ...
                      <li v-if="dataPerfil.rol?.id !== 3 && dataPerfil.rol?.id !== 4"><button class="btn btn-outline-warning text-red dropdown-item fs-5 p-0" title="Exportar" type="button" @click="fileData(row,'export','inventario' )"><i class="bi bi-upload"></i>Exportar</button></li>
    ...
    <EliminarModalSinPerifericos :handleData="handleData" :paramsE="paramsE" :isLoadingImport="isLoadingImport" />

    <MostrarModalSinPerifericos :paramsE="paramsE" />

    <ImportarModalSinPerifericos :isLoadingImport="isLoadingImport" :fileData="fileData" :response="response" />
