            ...
            <table class="table table-secondary table-striped table-hover" id="dataTable">
              ...
              <tbody>
                <tr v-for="(row, index) in filteredAndPaginatedData" :key="index">
                  <div class="form-check form-switch d-flex justify-content-end px-4" style="height: 60px;" v-if="dataPerfil.rol.id !== 2 && dataPerfil.rol.id !== 3">
                  ...
                  <td>
                    ...
                    <ul class="dropdown-menu p-2 gap-3">
                      <li v-if="dataPerfil.rol.id !== 3"><button class="btn btn-outline-secondary text-red dropdown-item fs-5 p-0" title="PDF" type="button" data-bs-toggle="modal" data-bs-target="#staticPDF" @click="handleData('fetch', '', row.id)"><i class="bi bi-file-pdf"></i>PDF</button></li>
                      <li v-if="dataPerfil.rol.id !== 3"><button class="btn btn-outline-warning text-red dropdown-item fs-5 p-0" title="Exportar" type="button" @click="fileData(row,'export',row.producto?.nombre)"><i class="bi bi-upload"></i>Exportar</button></li>
