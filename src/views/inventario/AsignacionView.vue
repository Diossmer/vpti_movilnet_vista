...
import AgregarModalAsignar from '@/components/modales/inventario/asignacion/AgregarModalAsignar.vue';
import EditarModalAsignar from '@/components/modales/inventario/asignacion/EditarModalAsignar.vue';
import MostrarModalAsignar from '@/components/modales/inventario/asignacion/MostrarModalAsignar.vue';
import ImportarModalAsignar from '@/components/modales/inventario/asignacion/ImportarModalAsignar.vue';
import PdfModalAsignar from '@/components/modales/inventario/asignacion/PdfModalAsignar.vue';
import EliminarModalAsignar from '@/components/modales/inventario/asignacion/EliminarModalAsignar.vue';
  ...
  filteredData.value = rowData.value.filter(row => {
    ...
    if (globalQuery) {
      const standardMatch = Object.values(row).some(cell =>
        ...
        String(row.usuario.usuario)?.toLowerCase().includes(globalQuery)
      );
    ...
    return Object.values(row).every((cell, index) => {
      ...
      return Object.values(row).every(cell =>{
        return String(row.fecha_asignar)?.includes(searchQuery)||
        String(row.fecha_devolucion)?.includes(searchQuery)||
...
const handleData = async (action = null, params = null, id = null) => {
  try {
    if (action === 'create' || action === 'update') {
      ...
      response.value = action=== 'create'? await AsignacionServicios(action, id, params)
    : action === 'update'? await AsignacionServicios(action, id, params):undefined;
      await handleData('fetchAll');
    } else if (action === 'delete') {
      await AsignacionServicios(action, id);
      await handleData('fetchAll');
    } else if (action === 'fetch') {
      paramsE.value = await AsignacionServicios('fetch', id);
    }
  } catch (error) {
...
const fileData = async (fileEvent,format,nameFile) => {
  switch (format) {
    case 'pdf':
      await AsignacionServicios(format,fileEvent.id,nameFile);
        break;
    case 'export':
      await AsignacionServicios(format,fileEvent.id,nameFile);
        break;
    ...
    case 'import':
      ...
      try {
          response.value = await AsignacionServicios(format, '', formData);
          await handleData('fetchAll');
      } catch (error) {
  ...
  try {
    if (type === 'individual' && selectedRows.value?.length > 0) {
      ...
      await Promise.all(selectedRows.value.map(row =>{
        if(row.id !==1){
          AsignacionServicios('delete', row.id)
        }
      }));
    } else if (type === 'all') {
...
<template>
  <div>
    ...
    <span class="badge fs-1"><i class="bi bi-person-badge-fill"></i> Asignaciones</span>
          ...
          <a type="button" class="btn btn-outline-secondary text-red" title="Agregar" data-bs-toggle="modal" data-bs-target="#staticAgregar">
           ...
           <div class="w-100 d-flex justify-content-between p-2">
              ...
              <div class="row d-flex justify-content-between">
                <div class="col-6 btn-group" v-if="dataPerfil.rol?.id !== 4">
                  <div class="dropdown" v-if="dataPerfil.rol?.id !== 2 && dataPerfil.rol?.id !== 3">
           ...
           <div class="table-responsive">
            <table class="table table-secondary table-striped table-hover" id="dataTable">
              <thead class="">
                <tr class="">
                  <div class="p-2 text-nowrap" v-if="dataPerfil.rol?.id !== 2 && dataPerfil.rol?.id !== 3 && dataPerfil.rol?.id !== 4"><i class="bi bi-ui-checks"></i></div>
                    <div class="form-check form-switch d-flex justify-content-end px-5" style="height: 60px;" v-if="dataPerfil.rol?.id !== 2 && dataPerfil.rol?.id !== 3 && dataPerfil.rol?.id !== 4">
              ...
              <tbody>
                <tr v-for="(row, index) in filteredAndPaginatedData" :key="index">
                  <div class="form-check form-switch d-flex justify-content-end px-4" style="height: 60px;" v-if="dataPerfil.rol?.id !== 2 && dataPerfil.rol?.id !== 3 && dataPerfil.rol?.id !== 4">
                  ...
                  <td>
                    ...
                    <ul class="dropdown-menu p-2 gap-3">
                      ...
                      <li v-if="dataPerfil.rol?.id !== 3 && dataPerfil.rol?.id !== 4"><button class="btn btn-outline-warning text-red dropdown-item fs-5 p-0" title="Exportar" type="button" @click="fileData(row,'export',row.producto?.nombre)"><i class="bi bi-upload"></i>Exportar</button></li>
    ...
    <AgregarModalAsignar :handleData="handleData" :relations="relations" :isLoadingImport="isLoadingImport" :response="response" />

    <EditarModalAsignar :handleData="handleData" :paramsE="paramsE" :relations="relations" :isLoadingImport="isLoadingImport" :response="response" />

    <EliminarModalAsignar :handleData="handleData" :paramsE="paramsE" :isLoadingImport="isLoadingImport" />

    <MostrarModalAsignar :paramsE="paramsE" :relations="relations" />

    <ImportarModalAsignar :isLoadingImport="isLoadingImport" :fileData="fileData"  :response="response" />
    
    <PdfModalAsignar :fileData="fileData" :paramsE="paramsE" />
