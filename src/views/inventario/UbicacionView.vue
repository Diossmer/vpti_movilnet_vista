...
import AgregarModalUbicacion from '@/components/modales/inventario/ubicacion/AgregarModalUbicacion.vue';
import EditarModalUbicacion from '@/components/modales/inventario/ubicacion/EditarModalUbicacion.vue';
import MostrarModalUbicacion from '@/components/modales/inventario/ubicacion/MostrarModalUbicacion.vue';
...
import EliminarModalUbicacion from '@/components/modales/inventario/ubicacion/EliminarModalUbicacion.vue';
...
const handleData = async (action = null, params = null, id = null) => {
  try {
    if (action === 'create' || action === 'update') {
      ...
      response.value = action=== 'create'? await UbicacionServicios(action, id, params)
    : action === 'update'? await UbicacionServicios(action, id, params):undefined;
      await handleData('fetchAll');
    } else if (action === 'delete') {
      await UbicacionServicios(action, id);
      await handleData('fetchAll');
    } else if (action === 'fetch') {
      paramsE.value = await UbicacionServicios('fetch', id);
    }
  } catch (error) {
...
const fileData = async (fileEvent,format,nameFile) => {
  switch (format) {
    ...
    case 'import':
      ...
      try {
          response.value = await UbicacionServicios(format, '', formData);
          await handleData('fetchAll');
      } catch (error) {
...
<template>
    ...
    <AgregarModalUbicacion :handleData="handleData" :relations="relations" :isLoadingImport="isLoadingImport" :response="response" />
    ...
    <ImportarModalUbicacion :isLoadingImport="isLoadingImport" :fileData="fileData" :response="response" />

    <PdfModalUbicacion :fileData="fileData" :paramsE="paramsE" />
