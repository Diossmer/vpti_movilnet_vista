...
import EliminarModalEvaluaciones from '@/components/modales/inventario/evaluacion/EliminarModalEvaluaciones.vue';
  ...
  filteredData.value = rowData.value.filter(row => {
    ...
    return Object.values(row).every((cell, index) => {
      ...
      return Object.values(row).every(cell =>{
        return String(row.escala)?.toLowerCase().includes(searchQuery)||
        String(row.compatibilidad)?.toLowerCase().includes(searchQuery)||
        String(row.reemplazo)?.toLowerCase().includes(searchQuery)||
        String(row.mantenimiento)?.toLowerCase().includes(searchQuery)
      });
    });
  });
...
const handleData = async (action = null, params = null, id = null) => {
  try {
    if (action === 'create' || action === 'update') {
      ...
      response.value = action=== 'create'? await EvaluacionesServicios(action, id, params)
    : action === 'update'? await EvaluacionesServicios(action, id, params):undefined;
      await handleData('fetchAll');
    } else if (action === 'delete') {
      await EvaluacionesServicios(action, id);
      await handleData('fetchAll');
    } else if (action === 'fetch') {
      paramsE.value = await EvaluacionesServicios('fetch', id);
    }
  } catch (error) {
    console.error('Error al manejar los datos:', error);
  } finally {
    isLoadingImport.value = false
    const {mostrarT,estatus,descripciones} = await EvaluacionesServicios('fetchAll');
...
const fileData = async (fileEvent,format,nameFile) => {
  switch (format) {
    ...
    case 'import':
      ...
      try {
          response.value = await EvaluacionesServicios(format, '', formData);
          await handleData('fetchAll');
      } catch (error) {
  ...
  <div>
    ...
    <div class="card w-100">
      <div class="card-body p-5">
        <div class="w-100 d-flex justify-content-end" v-if="dataPerfil.rol?.id !== 2 && dataPerfil.rol?.id !== 3 && dataPerfil.rol?.id !== 4">
          <a type="button" class="btn btn-outline-secondary text-red" title="Agregar" data-bs-toggle="modal" data-bs-target="#staticAgregar">
           ...
           <div class="w-100 d-flex justify-content-between p-2">
              ...
              <div class="row d-flex justify-content-between">
                <div class="col-6 btn-group" v-if=" dataPerfil.rol?.id !== 3 && dataPerfil.rol?.id !== 4">
                  ...
                  <div class="dropdown" v-if="dataPerfil.rol?.id !== 3 && dataPerfil.rol?.id !== 4">
                    ...
                    <ul class="dropdown-menu">
                      ...
                      <li v-if="dataPerfil.rol?.id !== 3"><button class="dropdown-item" type="button" @click="fileData(rowData,'exportAll','evaluaciones')"><i class="bi bi-upload"></i> Exportar</button></li>
                ...
                <tr v-for="(row, index) in filteredAndPaginatedData" :key="index">
                  ...
                  <td>
                    ...
                    <ul class="dropdown-menu p-2 gap-3">
                      ...
                      <li v-if="dataPerfil.rol?.id !== 3 && dataPerfil.rol?.id !== 4"><button class="btn btn-outline-warning text-red dropdown-item fs-5 p-0" title="Exportar" type="button" @click="fileData(row,'export','Evaluacion' )"><i class="bi bi-upload"></i>Exportar</button></li>
                      <li v-if="dataPerfil.rol?.id !== 3">
    ...
    <AgregarModalEvaluaciones :handleData="handleData" :relations="relations" :isLoadingImport="isLoadingImport" :response="response" />
    ...
    <ImportarModalEvaluaciones :isLoadingImport="isLoadingImport" :fileData="fileData" :response="response" />
