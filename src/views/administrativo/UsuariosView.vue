...
import EliminarModalUsuarios from '@/components/modales/administrativo/usuarios/EliminarModalUsuarios.vue';
...
const filteredAndPaginatedData = computed(() => {
  filteredData.value = rowData.value.filter(row => {
    ...
    return Object.values(row).every((cell, index) => {
      ...
      if (row.rol && row.rol.nombre && row.rol.nombre.toLowerCase().includes(searchQuery)) {
          return true;
      }
...
const handleData = async (action = null, params = null, id = null) => {
  try {
    if (action === 'create' || action === 'update') {
      ...
      response.value = action=== 'create'? await UsuariosServicios(action, id, params)
    : action === 'update'? await UsuariosServicios(action, id, params):undefined;
      await handleData('fetchAll');
    } else if (action === 'delete') {
      await UsuariosServicios(action, id);
      await handleData('fetchAll');
    } else if (action === 'fetch') {
      paramsE.value = await UsuariosServicios('fetch', id);
    }
  } catch (error) {
...
const fileData = async (fileEvent,format,nameFile) => {
  switch (format) {
    ...
    case 'import':
      ...
      try {
          response.value = await UsuariosServicios(format, '', formData);
          await handleData('fetchAll');
      } catch (error) {
...
<template>
  <div>
              ...
              <div class="col-6 btn-group" v-if="dataPerfil.rol?.id !== 2 && dataPerfil.rol?.id !== 4">
                ...
                <div class="dropdown">
                  ...
                  <ul class="dropdown-menu">
                    ...
                    <li v-if="dataPerfil.rol?.id !== 3"><button class="dropdown-item" type="button" @click="fileData(rowData,'exportAll','usuarios')"><i class="bi bi-upload"></i> Exportar</button></li>
                ...
                <tr v-for="(row, index) in filteredAndPaginatedData" :key="index">
                  ...
                  <td>
                    ...
                    <ul class="dropdown-menu p-2 gap-3">
                      ...
                      <li v-if="dataPerfil.rol?.id !== 2 && dataPerfil.rol?.id !== 3 && dataPerfil.rol?.id !== 4"><button class="btn btn-outline-warning text-red dropdown-item fs-5 p-0" title="Exportar" type="button" @click="fileData(row,'export',row.usuario)"><i class="bi bi-upload"></i>Exportar</button></li>
    ...
    <AgregarModalUsuarios :handleData="handleData" :relations="relations" :isLoadingImport="isLoadingImport" :response="response" />
    ...
    <ImportarModalUsuarios :isLoadingImport="isLoadingImport" :fileData="fileData" :response="response" />
